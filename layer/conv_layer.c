...
static void conv_layer_forward(layer_t *l)
{
	...
	for (b = 0; b < l->n->batch; ++b)
	{
		...
		gemm(0, 0, m, n, k, 1, &l->weight.val, 0, k, &conv->col.val, 0, n, 0, &l->out.val, b * l->out.size, n);
...
static void conv_layer_backward(layer_t *l)
{
	...
	for (b = 0; b < l->n->batch; ++b)
	{
		...
		gemm(0, 1, m, n, k, 1, &l->out.grad, b * l->out.size, k, &conv->col.val, 0, k, 1, &l->weight.grad, 0, n);

		m = conv->ic * conv->k * conv->k;
		n = conv->oh * conv->ow;
		k = conv->oc;

		gemm(1, 0, m, n, k, 1, &l->weight.val, 0, m, &l->out.grad, b * l->out.size, n, 0, &conv->col.grad, 0, n);
