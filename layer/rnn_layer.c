...
static void rnn_layer_forward(layer_t *l)
{
    ...
    for (t = 0; t < rnn->len; ++t)
    {
        ...
        memcpy(l->extra.val, l->out.val + out_offset, m * n * sizeof(data_val_t));

        gemm(0, 1, m, n, k, 1, &l->in.val, in_offset, k, &l->weight.val, 0, k, 0, &l->out.val, out_offset, n);
        gemm(0, 1, m, n, n, 1, &l->extra.val, 0, n, &l->weight.val, k * n, n, 1, &l->out.val, out_offset, n);
...
static void rnn_layer_backward(layer_t *l)
{
    ...
    for (t = rnn->len - 1; t >= 0; --t)
    {
        ...
        for (b = 0; b < k; ++b)
            for (i = 0; i < m; ++i)
            {
                l->out.grad[out_offset + b * m + i] += l->extra.grad[b * m + i];
            }

        gemm(1, 0, m, n, k, 1, &l->out.grad, out_offset, m, &l->in.val, in_offset, n, 1, &l->weight.grad, 0, n);
        gemm(1, 0, m, m, k, 1, &l->out.grad, out_offset, m, &l->extra.val, 0, m, 1, &l->weight.grad, n * m, m);
        ...
        gemm(0, 0, m, k, k, 1, &l->out.grad, out_offset, k, &l->weight.val, n * k, k, 0, &l->extra.grad, 0, k);
